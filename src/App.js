...
const BtnPrimary = ({ children, onClick, style = {}, disabled = false }) => (
  <button onClick={onClick} disabled={disabled} style={{ background: disabled ? '#aaa' : '#1D9E75', color: 'white', border: 'none', padding: '11px 16px', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: disabled ? 'not-allowed' : 'pointer', width: '100%', fontFamily: 'inherit', transition: 'background 0.15s', ...style }}>
    {children}
  </button>
);

const BtnSecondary = ({ children, onClick, style = {} }) => (
  <button onClick={onClick} style={{ background: '#f5f5f5', color: '#1a1a1a', border: '0.5px solid #ddd', padding: '8px 14px', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', ...style }}>
    {children}
  </button>
);
...
const StatCard    = ({ number, label, numberColor }) => (<div style={{ background: '#f9f9f9', borderRadius: 8, padding: '11px 13px' }}><div style={{ fontSize: 22, fontWeight: 500, color: numberColor || '#1a1a1a' }}>{number}</div><div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{label}</div></div>);
const LogItem     = ({ time, behavior, detail, dotColor }) => (<div style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: '0.5px solid #f0f0f0' }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: dotColor, marginTop: 4, flexShrink: 0 }} /><div><div style={{ fontSize: 11, color: '#888' }}>{time}</div><div style={{ fontSize: 13, fontWeight: 500, color: '#1a1a1a' }}>{behavior}</div><div style={{ fontSize: 11, color: '#888', marginTop: 1 }}>{detail}</div></div></div>);
...
const Spinner     = () => (<div style={{ textAlign: 'center', padding: 40, color: '#888', fontSize: 13 }}>Loading…</div>);
...
function intensityColor(n) {
  return { 1: '#5DCAA5', 2: '#5DCAA5', 3: '#EF9F27', 4: '#E24B4A', 5: '#E24B4A' }[n] || '#888';
}
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      await apiFetch('/api/auth/register', {
        method: 'POST',
        body: JSON.stringify({ firstName, lastName, email: regEmail, password: regPw, role }),
      });
  ...
  const inputStyle = {
    ...
    border: '0.5px solid #ddd', fontSize: 13, fontFamily: 'inherit',
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '36px 32px 24px', textAlign: 'center', borderBottom: '0.5px solid #f0f0f0' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 20, fontWeight: 600, color: '#1D9E75', letterSpacing: -0.5 }}>
          ...
          <>
            ...
            <div style={{ marginBottom: 20, position: 'relative' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                />
                <button onClick={() => setShowPw(v => !v)} style={{ position: 'absolute', right: 11, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}>
                  <EyeIcon on={showPw} />
                </button>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 16, fontSize: 12, color: '#aaa' }}>
              ...
              <button onClick={() => switchTab('register')} style={{ background: 'none', border: 'none', color: '#1D9E75', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, padding: 0 }}>
                Create one
              </button>
            </div>
          </>
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
              <div>
                ...
                <input type="text" placeholder="Maria" value={firstName} onChange={e => setFirstName(e.target.value)} style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Last name</label>
                <input type="text" placeholder="Santos" value={lastName} onChange={e => setLastName(e.target.value)} style={inputStyle} />
              </div>
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={labelStyle}>Email address</label>
              <input type="text" placeholder="you@school.edu" value={regEmail} onChange={e => setRegEmail(e.target.value)} style={inputStyle} />
            </div>
            <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginBottom: 12, position: 'relative' }}>
              <label style={labelStyle}>Password <span style={{ color: '#bbb', fontWeight: 400 }}>(min 8 characters)</span></label>
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ ...inputStyle, paddingRight: 40 }}
                />
                <button onClick={() => setShowRegPw(v => !v)} style={{ position: 'absolute', right: 11, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex' }}>
                  <EyeIcon on={showRegPw} />
                </button>
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              ...
              <input
                ...
                style={{ ...inputStyle, borderColor: regPw2 && regPw !== regPw2 ? '#E24B4A' : '#ddd' }}
              />
            ...
            {regPw && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                  {[1,2,3].map(n => (
                    <div key={n} style={{ flex: 1, height: 3, borderRadius: 2, background: regPw.length >= n * 4 ? (n === 1 ? '#EF9F27' : n === 2 ? '#5DCAA5' : '#1D9E75') : '#e0e0e0' }} />
                  ))}
                </div>
                <div style={{ fontSize: 10, color: '#aaa' }}>
                  {regPw.length < 4 ? 'Weak' : regPw.length < 8 ? 'Fair' : 'Good'}
                </div>
              </div>
            )}

            <BtnPrimary onClick={handleRegister} disabled={loading || (regPw2 !== '' && regPw !== regPw2)}>
              {loading ? 'Creating account…' : 'Create account'}
            </BtnPrimary>

            <div style={{ fontSize: 11, color: '#bbb', textAlign: 'center', marginTop: 12, lineHeight: 1.5 }}>
              BCBA / admin accounts are created by your school administrator.
            </div>

            <div style={{ textAlign: 'center', marginTop: 12, fontSize: 12, color: '#aaa' }}>
              Already have an account?{' '}
              <button onClick={() => switchTab('login')} style={{ background: 'none', border: 'none', color: '#1D9E75', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, padding: 0 }}>
                Sign in
              </button>
            </div>
          </>
      ...
      <Topbar
        left={
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{new Date().toDateString()} · {user?.name || '…'}</div>
          </div>
        }
        right={
...
const SavedScreen = ({ navigate, student }) => (
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, textAlign: 'center' }}>
    ...
    <div style={{ fontSize: 13, color: '#888', lineHeight: 1.6, marginBottom: 24 }}>
      {student ? `Entry saved to ${student.first_name}'s timeline.` : 'Entry saved.'}
    </div>
                ...
                <div key={a.id} style={{ borderLeft: '3px solid #E24B4A', paddingLeft: 10, marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{cond.threshold && `≥${cond.threshold} incidents in ${cond.window_minutes || 60} min → notify ${action.notify_role || 'team'}`}</div>
...
const AddStudentScreen = ({ navigate, token, user }) => {
  ...
  if (!canAdd) return (
    <>
      ...
      <div className="scroll"><div style={{ background:'#FCEBEB', borderRadius:8, padding:'12px 14px', fontSize:13, color:'#A32D2D' }}>Only admins, teachers, and BCBAs can add students.</div></div>
    </>
  );
  ...
  const inputStyle = { width:'100%', padding:'9px 11px', borderRadius:8, border:'0.5px solid #ddd', fontSize:13, fontFamily:'inherit', outline:'none' };
  const labelStyle = { fontSize:11, color:'#888', display:'block', marginBottom:4 };
    ...
    <>
      ...
      <div className="scroll">
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:10 }}>
          <div><label style={labelStyle}>First name *</label><input type="text" placeholder="Emma" value={firstName} onChange={e => setFirstName(e.target.value)} style={inputStyle} /></div>
          <div><label style={labelStyle}>Last name *</label><input type="text" placeholder="Johnson" value={lastName} onChange={e => setLastName(e.target.value)} style={inputStyle} /></div>
        </div>
        <div style={{ marginBottom:10 }}><label style={labelStyle}>Date of birth</label><input type="date" value={dob} onChange={e => setDob(e.target.value)} style={inputStyle} /></div>
        <div style={{ marginBottom:10 }}>
          <label style={labelStyle}>Communication level</label>
          <select value={commLevel} onChange={e => setCommLevel(e.target.value)} style={{ ...inputStyle, background:'white' }}>
        ...
        <div style={{ marginBottom:14 }}><label style={labelStyle}>Reinforcers (comma separated)</label><input type="text" placeholder="fidget toys, music, stickers" value={reinforcers} onChange={e => setReinforcers(e.target.value)} style={inputStyle} /></div>
        <SectionLabel mt={0}>Sensory profile (tap to select)</SectionLabel>
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:14 }}>
          {sensoryOptions.map(s => (
            <button key={s} onClick={() => toggleSensory(s)} style={{ fontSize:11, padding:'5px 10px', borderRadius:20, cursor:'pointer', fontFamily:'inherit', background: sensory.includes(s) ? '#E1F5EE' : '#f5f5f5', color: sensory.includes(s) ? '#0F6E56' : '#666', border: sensory.includes(s) ? '1px solid #9FE1CB' : '0.5px solid #e0e0e0' }}>{s}</button>
          ))}
        </div>
        ...
        {iepGoals.map((goal, i) => (
          <div key={i} style={{ border:'0.5px solid #e8e8e8', borderRadius:10, padding:'10px 12px', marginBottom:8 }}>
            ...
            <input type="text" placeholder="e.g. Reduce elopement" value={goal.name} onChange={e => updateGoal(i,'name',e.target.value)} style={{ ...inputStyle, marginBottom:6 }} />
            <input type="text" placeholder="Target: ≤2x/day" value={goal.target} onChange={e => updateGoal(i,'target',e.target.value)} style={inputStyle} />
          </div>
        ))}
